...
@section('content')
    ...
    <x-adminlte-modal id="modalPurple" title="Formulario de Facultad" theme="primary" icon="fas fa-building" size='lg'>
        <form id="formulario">
    ...
    <script>
        ...
        $('#formulario').on('submit',(e)=>{
            ...
            if ($('#formulario')[0].checkValidity()) {
                ...
                if(idFacultad){
                    $.ajax({
                        url: `{{route('facultad.update',':id') }}`.replace(':id', idFacultad),
                        method: 'PUT',
                        data: formData,
                        headers: {
                            'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
                    ...
                    $.ajax({
                        url: "{{route('facultad.store')}}", // URL de tu ruta 'store'
                        method: 'POST',
                        data: formData,
                        headers: {
                            'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
        ...
        function eliminarRegistro(e){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let csrfToken = $('meta[name="csrf-token"]').attr('content');
                    $.ajax({
                        url: `{{ route('facultad.destroy', ':id') }}`.replace(':id', e),
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
                        },

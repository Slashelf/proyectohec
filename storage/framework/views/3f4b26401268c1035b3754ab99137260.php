        ...
        <div class="row d-flex flex-column align-items-center justify-content-center">
            <div class="col d-flex flex-column align-items-start justify-content-center" id="cabecera">
                <h1>
                    ...
                    Gestión de Facultades de las Universidades de Bolivia
                </h1>
                <div class="w-full d-flex justify-content-center align-items-center">
                    <span>
                        Bienvenido al administrador de facultades de las universidades de Bolivia
                    </span>
                    <button class="Btn ml-2"  data-toggle="modal" data-target="#modalPurple" onclick="modalresetearformulario()">
                        <div class="svgWrapper">
                            ...
                            <div class="text">Agregar Facultad</div>
                        </div>
                    </button>
                </div>
            </div>
...
<?php $component = JeroenNoten\LaravelAdminLte\View\Components\Tool\Modal::resolve(['id' => 'modalPurple','title' => 'Formulario de Facultad','theme' => 'primary','icon' => 'fas fa-building','size' => 'lg'] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $component->withAttributes([]); ?>
        <form id="formulario">
            ...
            <input type="text" name="facultad" placeholder="Ingresa la Facultad" id="facultad" class="form-control" required>
    ...
    <script>
        $(document).ready( function () {
            $('#tabla').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "<?php echo e(route('facultad.index')); ?>",
                    type: 'GET',
                    data: function(d) {
                        return $.extend({}, d, {
                            extraParam1: 'valor1',
            ...
            $('#departamento').on('select2:select', function (e) {
                ...
                $.ajax({
                    url: `<?php echo e(route('universidad.show',':id')); ?>`.replace(':id', departamento),  // Reemplaza con la ruta correcta en tu servidor
                    type: 'GET',
        ...
        $('#formulario').on('submit',(e)=>{
            ...
            if ($('#formulario')[0].checkValidity()) {
                ...
                let csrfToken = $('meta[name="csrf-token"]').attr('content');
                let idFacultad = $('#idFacultad').val();
                if(idFacultad){
                    $.ajax({
                        url: `<?php echo e(route('facultad.update',':id')); ?>`.replace(':id', idFacultad),
                        method: 'PUT',
                        data: formData,
                        headers: {
                            'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                position: "center",
                                icon: "success",
                                title: "La Facultad se actualizó correctamente",
                    ...
                    $.ajax({
                        url: "<?php echo e(route('facultad.store')); ?>", // URL de tu ruta 'store'
                        method: 'POST',
                        data: formData,
                        headers: {
                            'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                position: "center",
                                icon: "success",
                                title: "La Facultad se registró correctamente",
                                showConfirmButton: false,
                                timer: 1500
                            });
                            $('#cerrar').click();
        ...
        function eliminarRegistro(e){
            Swal.fire({
                title: "¿Estas seguro de eliminar la Facultad?",
                ...
                if (result.isConfirmed) {
                    let csrfToken = $('meta[name="csrf-token"]').attr('content');
                    $.ajax({
                        url: `<?php echo e(route('facultad.destroy', ':id')); ?>`.replace(':id', e),
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
                        },
                        success: function(response) {
                            ...
                            Swal.fire({
                                position: "center",
                                icon: "success",
                                title: response.nombreuniversidad+"La Facultad se eliminó correctamente",
                                showConfirmButton: false,
                                timer: 1500
                            });
                            $('#cerrar').click();
        ...
        function mostrarModal(id,facultad,idUniversidad, departamento){
            $('#facultad').val(facultad);
            
            // Actualizar el valor del select de departamento y disparar el evento change para que Select2 lo refleje
            ...
            // Actualizar el valor del select de universidad y disparar el evento change para que Select2 lo refleje
            $('#universidad').val(idUniversidad).trigger('change');


            // Actualizar el campo oculto de idUniversidad
            $('#idFacultad').val(id);
        }
    </script>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\Rubik\resources\views/administrador/facultades/facultades.blade.php ENDPATH**/ ?>
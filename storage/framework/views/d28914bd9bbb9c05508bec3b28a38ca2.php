...
<?php $component = JeroenNoten\LaravelAdminLte\View\Components\Tool\Modal::resolve(['id' => 'modalPurple','title' => 'Formulario de carrera','theme' => 'success','icon' => 'fas fa-user-graduate','size' => 'lg'] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
    ...
    <script>
        $(document).ready( function () {
            ...
            $('#tabla').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "<?php echo e(route('carrera.index')); ?>",
                    type: 'GET',
                    data: function(d) {
                        return $.extend({}, d, {
                            extraParam1: 'valor1',
            ...
            $('#universidad').on('select2:select', function (e) {
                ...
                $.ajax({
                    url: `<?php echo e(route('facultad.show',':id')); ?>`.replace(':id', idUniversidad),
                    type: 'GET',
                    success: function (data) {
                        $('#facultad').empty();
        ...
        $('#formulario').on('submit',(e)=>{
            ...
            if ($('#formulario')[0].checkValidity()) {
                ...
                if(idCarrera){
                    $.ajax({
                            url: `<?php echo e(route('carrera.update',':id')); ?>`.replace(':id', idCarrera),
                            method: 'PUT',
                            data: formData,
                            headers: {
                        ...
                        $.ajax({
                            url: "<?php echo e(route('carrera.store')); ?>", // URL de tu ruta 'store'
                            method: 'POST',
                            data: formData,
                            headers: {
                                'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
        ...
        function mostrarModal(id, nombreCarrera, departamento, idFacultad, idUniversidad){
            ...
            $.ajax({
                url: `<?php echo e(route('universidad.show',':id')); ?>`.replace(':id', departamento),
                type: 'GET',             
                success: function (data) {
                    $('#universidad').empty();
                    $('#universidad').append('<option></option>');
        ...
        function eliminarRegistro(e){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let csrfToken = $('meta[name="csrf-token"]').attr('content');
                    $.ajax({
                        url: `<?php echo e(route('carrera.destroy', ':id')); ?>`.replace(':id', e),
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken // Añade el token CSRF a los headers
                        },
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\Rubik\resources\views/administrador/carreras/carreras.blade.php ENDPATH**/ ?>